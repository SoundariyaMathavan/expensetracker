...
def get_summary():
    try:
        ...
        # Generate plot - enhanced semi-circle chart for big views
        plt.figure(figsize=(12, 8))  # Larger figure size
        plt.style.use('dark_background')  # Dark background for better visualization
        
        if not df.empty:
            plot_df = df.groupby('category')['amount'].sum().reset_index()
            if not plot_df.empty:
                # Create a semi-circle pie chart with enhanced styling
                wedges, texts, autotexts = plt.pie(
                    ...
                    wedgeprops={
                        ...
                        'linewidth': 1,
                        'antialiased': True,
                        'width': 0.6  # Makes it a donut chart
                    },
                    textprops={
                        'color': 'white', 
                        'fontweight': 'bold',
                    ...
                    counterclock=False,
                    frame=False,
                    explode=[0.05] * len(plot_df)  # Slight explosion for all wedges
                )
                
                # Add category labels with connecting lines
                plt.legend(
                    wedges,
                    plot_df['category'],
                    title="Categories",
                ...
                # Set aspect ratio to make it a semi-circle
        ...
        plt.title('Expenses by Category', color='white', fontsize=18, fontweight='bold')
        plt.tight_layout()
